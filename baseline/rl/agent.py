...
class Agent:
    ...
    def qtfirst(self, agent_id):
        ...
        if len(candi_list) == 0:
            ...
            if len(sorted_list) > 0:
                a = sorted(sorted_list, key=lambda x: (self.job_status[x]['remain_pending_time']/(self.job_status[x]['priority']+1),self.job_status[x]['remain_process_time']))[0]
            else:
    ...
    def ptfirst(self, agent_id):
        ...
        if len(candi_list) == 0:
            ...
            a = sorted(candi_list, key=lambda x: (self.job_status[x]['remain_pending_time'], self.job_status[x]['remain_process_time']))[0]
            return a
    ...
    def get_act_rule_prev_feature(self, agent_id):
        # 鍔ㄤ綔瑙勫垯涓嬪嵆灏嗗埌姝ょ被鏈哄櫒鐨刯obs锛堣繕鍦ㄤ笂涓€绫绘満鍣ㄤ腑work鎴栬€呰繕鏈猘rrive锛変腑鍙兘鍑虹幇鐨剄time瓒呮椂淇℃伅
        # length: 8
