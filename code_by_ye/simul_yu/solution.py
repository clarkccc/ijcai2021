    ...
    def act(self, machine_status, job_status, time, job_list):
        ...
        if time ==0 and self.test == False:
            ...
            if abs(diff) <= max(mean_time_c, mean_time_d)/10:
                ...
                # pdb.set_trace()
                def draw_infos_extract(draw_infos, job_stage, breakdown_info, t, machine_status, job_status):
                    # 'J0092': {'type': 'd', 'priority': 5, 'status': 'to_arrive', 'arrival': 162, 'op': 'D01', 'remain_process_time': 21, 'remain_pending_time': inf, 'machine': None}
                    for machine, process_info in machine_status.items():
                        job = process_info['job']
                        machine_type = process_info['type']
                        if job != None and job not in close_list[machine]:
                            close_list[machine].add(job)
                            pending_time = job_status_copy[job]['remain_pending_time']
                            if pending_time > process_info['remain_time']:
                                pending_time = process_info['remain_time']-1
                            elif -pending_time > process_info['remain_time']:
                                pending_time = -process_info['remain_time']+1
                            
                            if isinstance(job_stage[job][machine_type], bool):
                                job_stage[job][machine_type] = machine
                                if job_status[job]['priority'] == 0:
                                    pending_time = 0
                                else:
                                    pending_time /= 2
                                draw_infos[machine][job] =(t, process_info['remain_time']-abs(pending_time), job_status[job]['priority'], pending_time)
                            else:
                                if isinstance(job_stage[job].get(machine_type), str):
                                    job_stage[job][machine_type] = [job_stage[job][machine_type], machine]
                                else:
                                    job_stage[job][machine_type].append(machine)
                                last_machine = job_stage[job][machine_type][-2]

                                breakdown_info[job].append((draw_infos[last_machine][job], machine))

                                del draw_infos[last_machine][job]
                                if job_status[job]['priority'] == 0:
                                    pending_time = 0
                                else:
                                    pending_time /= 2
                ...
                while check_time_total > 0:
                    ...
                    if check_time_total == max_iter:
                        ...
                        for job in change_job_set:
                            ...
                            self.dynamic_priority_change = priority_dict_base
                    rewards = 0
                    draw_infos_extract(draw_infos, job_stage, breakdown_info, t, machine_status_copy, job_status)
