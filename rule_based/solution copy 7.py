...
# todo stage 3 
# todo maxpend low op solutiion
# todo overview
# todo breakdown priority check max pend low

# todo priority view all 
    ...
    def train(self, run_time):
        ...
        for conf in self.conf_list:
            ...
            for funcName,weights in mp.items():
                rewards = 0
                ptv=0
                agent=Agent(env.job_types,weights,None)
                done = False
                ...
                weights_log[machine_key][funcName].append({'r':rewards,'ptv':ptv})
                print(machine_key,funcName,rewards,ptv)
        for m_k in weights_log:
            log=weights_log[m_k]
            fname=sorted([(fName,-sum([r['r'] for r in reward])) for fName,reward in log.items()],key=lambda x:x[1])
            print(fname)
            selectWeights[m_k]=mp[fname[0][0]]
        return Agent(env.job_types,None,selectWeights)

class Agent:
    def __init__(self, job_types,weights={"C":1,"D":3},selectWeights={}):
        if(selectWeights):
            print(selectWeights)
        self.job_types = job_types
        ...
        self.beter_weights=selectWeights
        op_jp=[]
    ...
    def preferOP(self,machine_status, job_status):
        ...
        weights=deepcopy(self.default_weights)
        if(self.beter_weights):
            b=dict(Counter([m_info['type'] for m_info in  machine_status.values()]))
            machine_key=''.join([str(b[i]) for i in ['A','B','C','D']])
            if(machine_key in self.beter_weights):
                weights=deepcopy(self.beter_weights[machine_key])
        #==========================================
        #before breakdonw
        #all idle
        class Before:
